...
3. پرسیدن `BOT_TOKEN`، `BOT_USERNAME`، `ADMIN_IDS`، `DOMAIN`
4. تولید خودکار `WEBHOOK_SECRET`، `API_KEY`، `JWT_SECRET` و رمز پایگاه‌داده
   (و همگام نگه‌داشتن `DATABASE_URL` با `POSTGRES_PASSWORD`)
5. ساخت ایمیج‌ها، اجرای دیتابیس/ردیس، اجرای مایگریشن‌ها و بالا آوردن سرویس‌ها
6. تنظیم وب‌هوک تلگرام

## تنظیم دامنه و SSL (nginx + certbot)

فایل نمونهٔ `nginx/uploader-bot.conf` ترافیک را به `api:8000` می‌رساند و برای
certbot آماده است:
