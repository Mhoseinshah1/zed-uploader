...
داده‌ها**، فقط این دستور را اجرا کنید:

```bash
cd ~/zed-uploader && git pull && sudo bash update.sh
```

اسکریپت `update.sh` به‌صورت کاملاً خودکار (بدون پرسش) کد را می‌کشد، ایمیج‌ها را
بازسازی می‌کند، مایگریشن‌ها را اعمال می‌کند و سرویس‌ها را با ایمیج جدید بالا
می‌آورد. این اسکریپت هرگز `docker compose down -v` نمی‌زند، به `pgdata` و فایل
`.env` دست نمی‌زند، و اجرای مکرر آن امن است.
